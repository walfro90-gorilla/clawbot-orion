...
- **Extensión Orion Sync**: `v0.10.14` (manifest) — 0.10.12 re-activó el focus-hack SOLO para search (regresión SalesNav); 0.10.13 añadió `get_contact_info` (digest diario); 0.10.14 **arregla** ese scrape (sacaba 0/60): detección scoped + poll 16s + captura debug. Anti-ban: piso de invite por-cuenta (scheduler); focus-hack OFF para invite/FU/inbox. ⚠️ verificar versión real con `linkedin_accounts.ext_version` — el disco de cada máquina puede ir atrás (al 11-ago las 3 cuentas van 0.10.13, falta jalar 0.10.14).
