...
- **Extensión Orion Sync**: `v0.10.16` (manifest) — 0.10.12 re-activó el focus-hack SOLO para search (regresión SalesNav); 0.10.13 añadió `get_contact_info` (digest diario); 0.10.14→0.10.16 **arreglan** ese scrape que sacaba 0/60 (root cause: el overlay es página completa, no modal; se exige heading+datos juntos) — CONFIRMADO extrae email en vivo. Anti-ban: piso de invite por-cuenta (scheduler); focus-hack OFF para invite/FU/inbox. ⚠️ verificar versión real con `linkedin_accounts.ext_version` — el disco de cada máquina puede ir atrás (al 12-ago solo Wal va 0.10.16; Josh+Café en 0.10.13, falta jalar).
