...
- **Gobierna**: `apps/prometheus/package.json`, `apps/prometheus/scripts/test-*.js`, `apps/prometheus/test/*.test.mjs`
...
| **Reescribir los 18 self-checks bajo un framework "para ordenarlos"** | Trabajo puro sin lector, y se pierden los datos reales incrustados en cada uno | — (no hacer) |
...
**En contra**: la cadena de `npm run check` **se mantiene a mano** en `package.json`. Crece por acumulación y nadie la poda; un script que se olvide de encadenar no corre nunca y nadie se entera. Es deuda aceptada, y el día que estorbe la respuesta es un glob, no un framework.
...
3. **Encadénalo en `npm run check`.** Un script que no está en la cadena no existe.
4. Antes de dar por bueno un cambio en `apps/prometheus`: `npm run check -w apps/prometheus`.
