...
|---|---|---|
| `apps/prometheus/scheduler-extension.js` | sync de lista→`campaign_target_companies` (TTL 30min, `sort_order`), cursor de empresa (`last_searched_at nulls first`), cursor de puestos por empresa (`title_idx`, pase de `COMPANY_TITLES_PER_PASS=6`), válvula acotada (`dry_search_streak ≥ 5` → UNA title-only), autocuración (pase completo sin leads → re-resolver, máx 3), `getEligiblePendingCount` cuenta SOLO leads de la lista en modo empresa, picker `fromList > seniorityRank` | El grupo booleano `OR` NO funciona en free (texto literal → 0 resultados). La válvula vieja BORRABA la lista. El pool viejo congelaba las búsquedas. |
| `apps/prometheus/lib/extension-dispatch.js` | `dispatchSearch` (facet URN, sin geoUrn/minEmployees con empresa, `searchMode` free forzado con empresa), `dispatchResolveCompanies`, `dispatchReloadExtension`, `seniorityRank`, gates de versión | La URL SalesNav v1 es keywords-only y se come el scoping. Sin URN → `"nombre exacto"` en keywords. |
