...
6. **"Orion está caído" casi nunca es Orion** (outage 17-ago-2026, ver `docs/bitacora-operativa.md`): la app web sigue respondiendo en localhost mientras Supabase arrastra a todo lo demás. Orden de diagnóstico: (a) `pm2 list` + `curl localhost:3000` (suele dar `307` en ms), (b) query real a `/rest/v1/` desde prod **con timing**, (c) `execute_sql` del MCP para separar capas — el MCP va por **Postgres directo** y la app por **PostgREST**; si el MCP responde y el REST timeoutea, lo roto es PostgREST. Si la DB está mal, **parar `prometheus-scheduler` + `extension-bridge` primero**: su bucle de reintentos re-satura la instancia e impide que levante. **NUNCA `pause_project`/`restore_project`** (lo único que expone el MCP): en Free tier el restore trae el último backup ⇒ pérdida de datos. Ojo: los logs del bridge dicen `auth failed` / `db: <empty>`, pero es un **timeout disfrazado** — las keys están bien.
