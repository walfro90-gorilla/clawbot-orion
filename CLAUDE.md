...
**Accept-detection = PRESENCIA, no ausencia (15-ago-2026, `6e0b586`)**: `invite_sent → connected` lo decide **solo `check_connections`** (el lead aparece en la lista de conexiones). `ingestCheckSentInvites` **ya NO infiere accept porque el lead desapareció de `/sent/`**: esa ausencia es ambigua — aceptó *o* la invitación nunca existió (invite fantasma; ver caso Josh abajo), y esa inferencia fabricaba falsos `connected` → FU a no-conexiones (76 en 2 días) → churn `detected_not_first_degree`. **Red de seguridad**: si la cuenta no tuvo un `check_connections` con datos en 24h, se conserva la inferencia conservadora (dejar accepts reales sin FU es el fallo caro — caso 8-jun). Al tocar esto, verificar SIEMPRE que `check_connections` de esa cuenta devuelva conexiones: si timeoutea (le pasaba a Josh: 120s → 0 conexiones), la detección positiva está muerta.
...
3. Con empresa: **sin `geoUrn` en la URL** (chocaba con el facet → 0 resultados), sin `minEmployees`. **(0.10.10)** El `searchMode` respeta la cuenta: `sales_navigator` usa el filtro nativo `CURRENT_COMPANY` en la URL de SalesNav (ve TODA la plantilla, no solo tu red — probado en vivo); `free` usa el facet. Empresa **sin URN** resuelto → siempre free con `"nombre exacto"`.
   ⚠️ **(11-ago) FILTRO DE GEO en el ingest** (`matchesCampaignGeo` en `extension-bridge.js`): como la URL tira el geoUrn, una empresa MULTINACIONAL de la lista (hunter douglas, MOLEX, Siemens) surfacea empleados de Brasil/Alemania/India. `ingestSearch` descarta al insertar los leads cuya ubicación nombra un país FUERA de `campaigns.search_location`. Chokepoint server-side → cubre free Y SalesNav sin tocar los scrapers. CONSERVADOR: reconoce el país por nombre (canon es/en) y descarta solo si es extranjero explícito; ciudad/región sin país ("Área metropolitana de San Luis Potosí") o location vacía → se conserva (el facet ya garantiza la empresa). Self-check `scripts/test-geo-filter.js`.
...
- **Proveedores** (`lib/ai-message.js`, jul-2026): punto único **`callLLM` / `callLLMJson`** con cadena `LLM_PROVIDERS` (default `groq,gemini`) → **Groq primario** (`llama-3.3-70b-versatile`, API OpenAI-compatible) + **Gemini 2.5 Flash de fallback** + template literal como red final. Config por env `LLM_PROVIDERS` / `GROQ_MODEL`. Mata la fragilidad de proveedor único (un billing caído no rompe la generación). El post-proceso (truncado por oración, guard anti-placeholder) es agnóstico de proveedor.
  ⚠️ **(17-ago-2026) la cadena estuvo CAÍDA COMPLETA y degradó en silencio** a template literal (FU) y a cero respuestas (FM, 51 replies sin contestar): Groq **retiró `llama-3.3-70b-versatile`** (`groq_404 … does not exist or you do not have access`) y el `openrouter/free` que se había puesto de respaldo agotó su cuota diaria. Prod quedó en **`LLM_PROVIDERS=groq,gemini,openrouter` + `GROQ_MODEL=openai/gpt-oss-120b`** (validado en vivo: DM en español limpio y JSON-mode correcto). **Gemini NO sirve de primario**: free tier = **20 requests/día** por modelo (`GenerateRequestsPerDayPerProjectPerModel-FreeTier`) y además da `FAILED_PRECONDITION: User location is not supported` desde el datacenter. Antes de "arreglar el prompt" cuando la IA no responde, verifica el modelo: `curl -s https://api.groq.com/openai/v1/models -H "Authorization: Bearer $GROQ_API_KEY"` lista los vigentes (evita `qwen/qwen3.6-27b`: filtra `<think>` al mensaje). ⚠️ **La key de Groq es de PAGA** (`x-ratelimit-limit-requests: 500000`, 250k TPM) — el 404 dice *"does not exist **or you do not have access to it**"* y esa segunda mitad hace pensar en un problema de plan o de key: **no lo es, el modelo se retiró**. Groq es el proveedor primario de pago. **(17-ago, tarde) Se sumó Kimi/Moonshot como respaldo TAMBIÉN de pago** — antes, si Groq caía, la red eran dos free tier ya agotados. Cadena en prod: **`LLM_PROVIDERS=groq,moonshot,gemini,openrouter`**. Kimi (`kimi-k3`, host GLOBAL `api.moonshot.ai` — el `.cn` rechaza la key) trae dos rarezas ya codificadas en `OPENAI_COMPAT`: **`fixedTemperature: 1`** (cualquier otro valor da 400) y **`tokenMultiplier: 4`** (sus tokens de razonamiento se comen el cap → el JSON llegaba truncado). Va de segundo y no de primario porque tarda ~9-13s (Groq <1s) y topa en **3 RPM por organización**.
  ⚠️ **(24-ago-2026) SEGUNDA caída silenciosa de la misma cadena, y esta sí llegó a clientes.** `gpt-oss-120b` es un modelo de **razonamiento**: con `max_tokens: 500` el razonamiento se comía el presupuesto ⇒ `finish_reason: length` ⇒ `content` **vacío** ⇒ **Groq falló 233 de 233** desde el 21-ago. Moonshot tapó 135 (revienta con `429 … max organization concurrency: 1`, que es concurrencia 1, no 3 RPM) y las otras **50 las contestó `openrouter/free`** — que **NO es un modelo, es un alias que rutea a un modelo gratis DISTINTO en cada llamada** (medido: `dots-3-note-preview` → vacío, `nemotron-3-super-120b` → 2026 chars de razonamiento crudo, `nemotron-3-nano` → bien). **Cuatro leads reales recibieron la cadena de pensamiento del modelo** (*"We need to produce a single response (max 320 characters…)"*, *"Vamos a analizar la situación paso a paso. 1. \*\*Historial…\*\*"*). Salió porque **la truncación corría ANTES del guard anti-meta**: se quedaba con la CABEZA (el análisis) y tiraba la respuesta real, y el resultado pasaba todos los guards (termina en punto, cabe en el cap, sin placeholders). Fix: `tokenMultiplier: 4` + `reasoningEffort: 'low'` en la config de groq (verificado en vivo, 8/8 limpias; cada uno basta por separado), guard anti-meta ensanchado a prosa de razonamiento y markdown, y **corrido sobre el texto crudo antes de truncar**. `openrouter` **fuera de la cadena**: prod queda en **`LLM_PROVIDERS=groq,moonshot,gemini`**. Si todo cae, el FU usa template verbatim y el auto-reply se salta — callar es preferible a mandar basura. Self-check `scripts/test-reply-guards.js` (lleva los 4 textos reales). Ground truth: ADR-0001 §6 y §7.
