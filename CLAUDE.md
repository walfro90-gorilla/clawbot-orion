...
⚠️ **Invites fantasma (caso Josh, 8-ago→)**: LinkedIn puede **descartar en silencio los envíos automatizados de una cuenta** (los manuales de esa misma cuenta sí entran) — probado con experimento controlado contra Café a la misma hora. El modal se cierra igual que en un envío bueno, así que **el cierre de modal NO prueba envío**; `send_invite` reporta `confirmedBy: 'toast' | 'modal_closed'` para distinguirlo. Contención: `linkedin_accounts.invites_paused_until` (pausa SOLO invites; FU/inbox siguen). Ground truth completo: memoria `weekly-invite-limit-ago2026`.
