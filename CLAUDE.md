...
- **Guard anti-contacto-inventado** (`findUnapprovedContact`, 01-sep-2026) — mismo chokepoint: un teléfono/email/URL en el output que NO aparezca en el propio prompt (config + `cal_com_url` + historial del lead) es fabricado y no se envía (el FM le inventó `+52 1 81 5555 1234` y un calendly roto a leads reales). En FM, `invented_contact` **pausa el lead + alerta `manual_reply_needed`** (el dato faltante lo pone un humano, no otro retry). Regla gemela en prompt: `NO_INVENT_CONTACT_RULE` (tampoco prometer llamar/enviar material). Cuenta sin `cal_com_url` = el bot no tiene NADA que dar cuando piden cita/teléfono — configúralo por cuenta en `linkedin_accounts`. Self-check: `scripts/test-reply-guards.js`.
