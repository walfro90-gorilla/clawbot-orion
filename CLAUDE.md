...
4. **`currentCompany` histórico null**: la regex `headlineCompany` (`worker.js`) que lo mitigaba es **código MUERTO**. En la ruta viva la regla "no inventes empresa / no uses el título como empresa" es texto en el prompt (`NO_INVENT_COMPANY_RULE`), ver §7.
